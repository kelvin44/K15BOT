...
*/
const BotName = '👾kelvin Bot👾'; // Nama Bot Whatsapp
...
conn.on('user-presence-update', json => console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @Hendra.stror`))
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @Hendra.stror`)
})
...
conn.sendMessage(id, 'Owner hendra wa.me/6282147078449' ,MessageType.text);
}
